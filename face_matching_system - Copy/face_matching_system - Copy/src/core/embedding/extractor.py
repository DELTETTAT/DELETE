...
class EmbeddingExtractor:
    ...
    def extract_single_embedding(self, image_path: str, enforce_detection: bool = True) -> Optional[np.ndarray]:
        ...
        try:
            ...
            fallback_attempts = [
                ...
                lambda: DeepFace.represent(
                    img_path=image_path,
                    model_name='Facenet',  # Smaller, more stable model
            ...
            for i, attempt in enumerate(fallback_attempts):
                try:
                    ...
                    self.logger.debug(f"Fallback {i+1} failed: {fallback_error}")
                    continue

            self.logger.error(f"All fallback attempts failed for {image_path}")
            return None
    ...
    def extract_batch_embeddings(self, 
        ...
        """
        Extract embeddings from multiple images in parallel using process-based parallelism
        ...
        # Optimize worker count - use more workers for CPU-intensive tasks
        if max_workers is None:
            max_workers = min(mp.cpu_count(), len(image_paths), mp.cpu_count() - 1 or 1)

        self.logger.info(f"Processing {len(image_paths)} images with {max_workers} process workers")
        start_time = time.time()

        embeddings = []
        successful_paths = []

        # Use ProcessPoolExecutor for CPU-intensive tasks (true parallelism)
        try:
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                # Submit all tasks with worker function
                future_to_path = {
                    executor.submit(
                        _extract_embedding_worker_process, 
                        path, 
                        self.model_name, 
                ...
                for future in as_completed(future_to_path):
                    ...
                    try:
                        embedding = future.result(timeout=60)  # Increased timeout
            ...
            self.logger.error(f"ProcessPoolExecutor failed: {e}")
            # Fallback to sequential processing
            self.logger.info("Falling back to sequential processing...")
            return self._extract_batch_sequential(image_paths, enforce_detection)
