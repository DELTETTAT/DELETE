...
class AdvancedImageEnhancer:
    ...
    def auto_enhance_image(self, image: np.ndarray, quality_metrics: Dict = None) -> np.ndarray:
        """
        Conservative enhancement that preserves facial features
        ...
        try:
            ...
            # Only apply conservative enhancements
            enhanced_image = image.copy()

            # Conservative lighting correction only
            enhanced_image = self._apply_conservative_lighting(enhanced_image, quality_metrics)

            return enhanced_image

        except Exception as e:
            self.logger.warning(f"⚠️ Conservative enhancement failed: {e}")
            return image

    def _apply_conservative_lighting(self, image: np.ndarray, quality_metrics: Dict) -> np.ndarray:
        """Apply only conservative lighting corrections"""
        try:
            brightness = quality_metrics.get('brightness', 127)

            # Only correct severe lighting issues
            if brightness < 60:  # Very dark
                # Gentle brightening
                pil_image = Image.fromarray(image)
                enhancer = ImageEnhance.Brightness(pil_image)
                enhanced = enhancer.enhance(1.3)  # Modest enhancement
                return np.array(enhanced)
            elif brightness > 220:  # Very bright
                # Gentle darkening
                pil_image = Image.fromarray(image)
                enhancer = ImageEnhance.Brightness(pil_image)
                enhanced = enhancer.enhance(0.8)  # Modest reduction
                return np.array(enhanced)

            return image

        except Exception as e:
            self.logger.warning(f"Conservative lighting correction failed: {e}")
            return image
