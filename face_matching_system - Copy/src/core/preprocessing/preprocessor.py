...
class AdvancedImagePreprocessor:
    ...
    def preprocess_image(self, input_path: str, output_path: Optional[str] = None) -> Dict[str, Any]:
        """
        Advanced preprocessing pipeline with automatic optimization
        ...
        try:
            ...
            initial_quality = self.quality_assessor.assess_image_quality(image)

            # Automatic enhancement based on quality metrics
            if self.enable_enhancement and self.enhancer:
                enhanced_image = self.enhancer.auto_enhance_image(image, initial_quality)
                if enhanced_image is not None:
                    image = enhanced_image

            # Advanced face alignment with multiple detection methods
            if self.enable_face_alignment and self.aligner:
                aligned_image = self.aligner.align_face_advanced(image)
                if aligned_image is not None:
                    image = aligned_image
                else:
                    self.logger.warning(f"⚠️ Advanced face alignment failed for {os.path.basename(input_path)}")

            # Intelligent resizing with aspect ratio preservation
            image = self._intelligent_resize(image, self.target_size)

            # Advanced normalization
            image = self._advanced_normalize(image)

            # Final quality assessment
            final_quality = self.quality_assessor.assess_image_quality(image)
            ...
            return {
                ...
                'quality_improvement': final_quality.get('overall_score', 0) - initial_quality.get('overall_score', 0),
                'processing_config': self.processing_config
            }

        except Exception as e:
            self.logger.error(f"❌ Advanced preprocessing failed for {input_path}: {e}")
