...
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Disable oneDNN optimizations to avoid CPU warnings
import tensorflow as tf

# Configure TensorFlow for compatibility - simplified approach
try:
    ...
    # Ensure eager execution is enabled (default in TF 2.x)
...
class EmbeddingExtractor:
    ...
    def extract_batch_embeddings(self, 
        ...
        Args:
            ...
            enforce_detection: Whether to enforce face detection
            max_workers: Maximum number of worker processes (optimized for CPU cores)
        ...
        # Optimize worker count based on CPU cores and image count
        if max_workers is None:
            max_workers = min(mp.cpu_count(), len(image_paths), 4)  # Cap at 4 for stability
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, future in enumerate(as_completed(future_to_path)):
                ...
                try:
                    embedding = future.result(timeout=30)  # 30 second timeout per image
