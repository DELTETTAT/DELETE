...
class EmbeddingExtractor:
    ...
    def extract_single_embedding_with_preprocessing(self, image_path: str, enforce_detection: bool = True, use_preprocessing: bool = False) -> Optional[np.ndarray]:
        """
        Extract embedding from a single image with preprocessing option
        ...
        try:
            if use_preprocessing:
                ...
                try:
                    ...
                    # Apply preprocessing
                    preprocess_result = preprocessor.preprocess_image(temp_path, temp_path)
                    
                    if preprocess_result.get('success', False):
                        # Extract from preprocessed image
                        return self.extract_single_embedding(temp_path, enforce_detection)
                    else:
                        # Fall back to original if preprocessing failed
            ...
            self.logger.error(f"Failed to extract embedding with preprocessing: {e}")
            return None
