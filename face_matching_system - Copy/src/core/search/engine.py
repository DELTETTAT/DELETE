...
class SearchEngine:
    ...
    def search_similar_faces(self, 
        ...
        try:
            ...
            # Check if index was built with preprocessing
            index_info = self.index_manager.get_index_info()
            preprocessing_used = index_info.get("preprocessing_used", False)

            # Preprocess query image if index was built with preprocessing
            processed_query_path = query_path
            if preprocessing_used:
                # Create temporary processed image
                import tempfile
                temp_dir = tempfile.mkdtemp()
                processed_query_path = os.path.join(temp_dir, "query_processed.jpg")

                # Apply same preprocessing as used during index building
                if not self.preprocessor.preprocess_image(query_path, processed_query_path):
                    self.logger.warning(f"Query preprocessing failed, using original image: {query_path}")
                    processed_query_path = query_path
                else:
                    self.logger.debug(f"Query image preprocessed for consistency with index")

            # Extract embedding for query image (preprocessed if needed)
            query_embedding = self.embedding_extractor.extract_single_embedding(
                processed_query_path, enforce_detection
            )

            # Cleanup temporary file if created
            if processed_query_path != query_path and os.path.exists(processed_query_path):
                try:
                    import shutil
