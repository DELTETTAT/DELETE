...
class SearchEngine:
    ...
    def search_similar_faces(self, 
        ...
        try:
            ...
            # Extract embedding from query image with preprocessing option
            if use_preprocessing is not None:
                # Use specific preprocessing setting
                query_embedding = self.embedding_extractor.extract_single_embedding_with_preprocessing(
                    image_path=query_path,
                    enforce_detection=enforce_detection,
                    use_preprocessing=use_preprocessing
                )
            else:
                # Use auto-detection based on index
            ...
            for i, (distance, idx) in enumerate(zip(distances[0], indices[0])):
                if idx < len(self.labels):
                    ...
                    similarity_score = max(0.0, min(1.0, np.exp(-distance / 200.0)))
                    
                    # Alternative: Linear similarity (distance-based)
                    # similarity_score = max(0.0, min(1.0, (1000 - distance) / 1000))
