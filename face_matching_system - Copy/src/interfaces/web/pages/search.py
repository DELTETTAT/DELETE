...
def render_upload_search_tab(orchestrator, stats):
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            # Search parameters
            k = st.slider("🎯 Number of Results", min_value=1, max_value=50, value=10, help="How many similar faces to find")
            threshold = st.number_input("📏 Distance Threshold", min_value=0.0, max_value=1000.0, value=450.0, step=10.0, help="Maximum distance for matches (lower = more strict)")
            
            # Advanced options
            with st.expander("🔧 Advanced Options"):
                use_preprocessing = st.checkbox("🎨 Enhance Image Quality", value=True, help="Apply image enhancement before search")
                enforce_detection = st.checkbox("👤 Require Face Detection", value=True, help="Only search if a face is detected")
                similarity_method = st.selectbox("📊 Similarity Method", ["L2 Distance", "Cosine Similarity"], help="Method for calculating similarity")
            
            # Search button
            if st.button("🔍 Search Similar Faces", type="primary", use_container_width=True):
                search_with_uploaded_image(orchestrator, uploaded_file, k, threshold, use_preprocessing, enforce_detection, stats)
...
def render_browse_search_tab(orchestrator, fs_ops, settings, stats):
    ...
    if 'selected_browse_image' in st.session_state:
        ...
        with col2:
            ...
            k = st.slider("🎯 Number of Results", min_value=1, max_value=50, value=10, key="browse_k")
            threshold = st.number_input("📏 Distance Threshold", min_value=0.0, max_value=1000.0, value=450.0, step=10.0, key="browse_threshold")
            
            with st.expander("🔧 Advanced Options"):
                use_preprocessing = st.checkbox("🎨 Enhance Image Quality", value=True, key="browse_preprocess")
                enforce_detection = st.checkbox("👤 Require Face Detection", value=True, key="browse_detection")
            
            col1, col2 = st.columns(2)
            with col1:
                if st.button("🔍 Search", type="primary", use_container_width=True):
                    search_with_file_path(orchestrator, image_path, k, threshold, use_preprocessing, enforce_detection, stats)
            with col2:
...
def search_with_uploaded_image(orchestrator, uploaded_file, k, threshold, use_preprocessing, enforce_detection, stats):
    ...
    try:
        ...
        if result['success']:
            display_search_results(result, stats, uploaded_file.name)
        else:
...
def search_with_file_path(orchestrator, image_path, k, threshold, use_preprocessing, enforce_detection, stats):
    ...
    if result['success']:
        display_search_results(result, stats, os.path.basename(image_path))
    else:
        st.error(f"❌ Search failed: {result.get('error', 'Unknown error')}")
...
def display_search_results(result, stats, query_name):
    ...
    with col3:
        ...
        st.markdown('<div class="metric-container"><div class="metric-value">{:.1f}</div><div class="metric-label">Avg Distance</div></div>'.format(avg_distance), unsafe_allow_html=True)
    with col4:
        best_match = min(results, key=lambda x: x['distance']) if results else None
        best_distance = best_match['distance'] if best_match else 0
        st.markdown('<div class="metric-container"><div class="metric-value">{:.1f}</div><div class="metric-label">Best Match</div></div>'.format(best_distance), unsafe_allow_html=True)
...
def show_result_details(result, stats):
    ...
    with col2:
        ...
        if distance < 100:
            interpretation = "Excellent match - Very high similarity"
        elif distance < 200:
            interpretation = "Good match - High similarity"
        elif distance < 350:
            interpretation = "Fair match - Moderate similarity"
        elif distance < 500:
            interpretation = "Weak match - Low similarity"
        else:
            interpretation = "Poor match - Very low similarity"
        
        st.markdown(f"**Interpretation:** {interpretation}")
        
        # Progress bar for similarity
        similarity_pct = max(0, min(100, (1000 - distance) / 10))
